...
Markdown.dialects.Gruber = {
  block: {
    ...
    //20120131 kosaki added L650-697 ,based on list.
    table: (function( ){
      var any_table = "[|]",
          is_table_re = new RegExp( "^( {0,3})(" + any_table + ")" );

      return function( block, next ) {
        var m = block.match( is_table_re );
        if ( !m ) return undefined;
        ...
        for (var line_no=0; line_no < lines.length; line_no++) {
          ...
          tr = [ "tablerow" ];
          var trimed_l = l.trim(),
              splited_l = trimed_l.split('|');

          for (i=0; i < splited_l.length;i ++) {
            if (splited_l[i] !== undefined && splited_l[i] !== '' ) {
              if (splited_l[i].match(/^~/)){
                var tt = ["tableheader"]; 
                var re = this.processInline( splited_l[i].match(/~(.*)/)[1] );
              }else{
                var tt = ["tabledata"];
                var re = this.processInline( splited_l[i] );
              }

              while (re.length) tt.push( re.shift() );
              tr.push(tt);
            }
          }
    ...
    //on the construction
    //escaped: function escaped( block, next ) {
      //var m = block.match( /^.*?\{\{(.*?)\}\}/ );
      //if ( !m ) {return undefined;}
      //var jsonml = [ [ "escaped" ] ];
      //return [ ["escaped"].concat( this.processInline( m[1] ) ) ];
    //},
...
//20120201 kosaki added
Markdown.dialects.Gruber.inline["~"]  = strong_em("escaped", "~");
...
function render_tree( jsonml ) {
  ...
  while ( jsonml.length ) {
    var a3 = tag;
    content.push(tag == 'escaped' ? jsonml.shift() : arguments.callee( jsonml.shift()));
  }
  ...
  // 20120113 kosaki added single tag.
  if (tag == 'img' || tag == 'br'){
...
function convert_tree_to_html( tree, references, options ) {
  ...
  switch ( jsonml[ 0 ] ) {
    ...
    case "listitem":
      jsonml[ 0 ] = "li";
      break;
    //20120131 kosaki added table tags set L1416-1427
    //case "table":
      //jsonml[ 0 ] = "table";
      //break;
    case "tablerow":
